...
const format1BudgetTotals = computed(() => {
  ...
  result.assetsTotal = sumSection(currentReport.structure.Assets)
  result.liabilitiesTotal = sumSection(currentReport.structure.Liabilities)
  result.equityTotal = sumSection(currentReport.structure.Equity)

  const netIncomeBudget = currentReport.netIncomeBudget || 0
  result.totalEquity = result.equityTotal + netIncomeBudget
  result.totalLiabilitiesAndEquity = result.liabilitiesTotal + result.totalEquity
...
const tFormatRows = computed(() => {
  ...
  // Assets groups
  if (currentReport.structure.Assets && currentReport.structure.Assets.groups) {
    Object.entries(currentReport.structure.Assets.groups).forEach(([name, group]) => {
      assetsList.push({
        type: 'group',
        name,
        total: group.total,
  ...
  if (currentReport.structure.Liabilities && currentReport.structure.Liabilities.groups) {
    Object.entries(currentReport.structure.Liabilities.groups).forEach(([name, group]) => {
      liabEqList.push({
        type: 'group',
        name,
        total: group.total,
  ...
  liabEqList.push({
    ...
    total: currentReport.structure.Liabilities.total,
    budget: format1BudgetTotals.value.liabilitiesTotal,
  })

  // Equity Section Header
  liabEqList.push({ type: 'section', name: 'Equity' })
  if (currentReport.structure.Equity && currentReport.structure.Equity.groups) {
    Object.entries(currentReport.structure.Equity.groups).forEach(([name, group]) => {
      liabEqList.push({
        type: 'group',
        name,
        total: group.total,
  ...
  // Net Income row inside Equity
  liabEqList.push({
    type: 'group',
    name: 'Current Year Earnings',
    total: currentReport.netIncome,
    budget: currentReport.netIncomeBudget,
    items: [],
    isNetIncome: true,
  })
  // Subtotal Equity
  liabEqList.push({
    type: 'subtotal',
    name: 'Total Equity',
    total: currentReport.structure.Equity.total + currentReport.netIncome,
    budget: format1BudgetTotals.value.totalEquity,
  })

  const rowCount = Math.max(assetsList.length, liabEqList.length)
  const rows = []
  for (let index = 0; index < rowCount; index++) {
    rows.push({
      left: assetsList[index] || null,
      right: liabEqList[index] || null,
    })
  }
  return rows
})
...
const matrixRows = computed(() => {
  if (!matrixState.value) return { rows: [], totalLiabRow: null, totalEqRow: null }
  const rows = []
  let rowIdx = 2 // Row 1 is the column letters

  let assetsStart = null
  let assetsEnd = null
  let liabStart = null
  let liabEnd = null
  let eqStart = null
  ...
  Object.entries(matrixState.value.data).forEach(([sectionName, sectionData]) => {
    ...
    if (sectionName === 'Assets') {
      assetsStart = secStartIdx
      assetsEnd = secEndIdx
    } else if (sectionName === 'Liabilities') {
      liabStart = secStartIdx
      liabEnd = secEndIdx
    }
    ...
    if (sectionName === 'Equity') {
      ...
      rows.push({
        type: 'earnings',
        name: 'Current Year Earnings',
        totals: matrixState.value.netIncomes,
        budgets: matrixState.value.netIncomeBudgets,
        items: [],
        section: sectionName,
        rowNum: rowIdx++,
      })
      ...
      start:
        sectionName === 'Assets'
          ...
          : sectionName === 'Liabilities'
            ? liabStart
            : eqStart,
      end: sectionName === 'Assets' ? assetsEnd : sectionName === 'Liabilities' ? liabEnd : eqEnd,
      netIncRow: sectionName === 'Equity' ? netIncRow : null,
    })

    if (sectionName === 'Liabilities') {
      totalLiabRow = subtotalRow
    } else if (sectionName === 'Equity') {
      totalEqRow = subtotalRow
    }
  })
  ...
  sectionsArray.forEach((section) => {
    ...
    reportKeys.forEach((key) => {
      if (multiReports.value[key]?.structure[section]) {
        Object.keys(multiReports.value[key].structure[section].groups).forEach((group) =>
          allGroups.add(group),
        )
      }
    })
      ...
      .forEach((groupName) => {
        ...
        reportKeys.forEach((key) => {
          if (multiReports.value[key]?.structure[section]?.groups[groupName]) {
            multiReports.value[key].structure[section].groups[groupName].items.forEach((item) =>
              allItems.add(item.no + '::' + item.name),
            )
          }
        })
        ...
        const groupBudgets = reportKeys.map((key, index) =>
          itemsList.reduce((sum, item) => sum + item.budgets[index], 0),
        )

        result[section].groups[groupName] = {
          items: itemsList,
          totals: reportKeys.map(
            (key) => multiReports.value[key]?.structure[section]?.groups[groupName]?.total || 0,
          ),
          budgets: groupBudgets,
        }
      })

    result[section].totals = reportKeys.map(
      (key) => multiReports.value[key]?.structure[section]?.total || 0,
    )
    result[section].budgetTotals = reportKeys.map((key, index) => {
      return Object.values(result[section].groups).reduce(
        (sum, group) => sum + group.budgets[index],
        0,
      )
    })
  })
...
const matrixState = computed(() => {
  ...
  if (displayFormat.value === '2') {
    ...
    const getQuarterSub = (quarter, currentPeriod) => {
      if (quarter === 1)
        return currentPeriod >= 3 ? 'JAN-MAR' : currentPeriod === 1 ? 'JAN' : 'JAN-FEB'
      if (quarter === 2)
    ...
    baseHeaders = [
      { label: 'Q1', sub: getQuarterSub(1, period) },
      { label: 'Q2', sub: getQuarterSub(2, period) },
      { label: 'Q3', sub: getQuarterSub(3, period) },
      { label: 'Q4', sub: getQuarterSub(4, period) },
    ]
...
<template>
  ...
  >
    <template #controls>
      <div
        v-if="isAuthorized && availableYears.length > 0"
        class="flex items-center justify-start gap-1.5 w-full"
      >
        <div
        ...
        >
          <select
            ...
            :disabled="displayFormat === '4'"
            class="bg-transparent text-xs font-semibold text-slate-700 px-2 py-0.5 focus:outline-none cursor-pointer hover:bg-sky-50 transition-colors disabled:opacity-50 disabled:bg-slate-100"
          >
            <option v-for="period in availablePeriods" :key="period" :value="period">
              {{ monthNames[period] || period }}
            </option>
          </select>
        ...
        <div
          class="flex items-center gap-1 bg-white border border-sky-200 rounded px-2 py-0.5 shadow-sm shrink-0"
        >
          <label
            class="flex items-center gap-1 cursor-pointer text-xs font-semibold text-[#107c41] whitespace-nowrap"
          >
            <input
              type="checkbox"
              v-model="showBudget"
              class="w-3 h-3 text-[#107c41] rounded border-slate-300 focus:ring-[#107c41] cursor-pointer"
            />
            Budget
          </label>
        </div>
        ...
        <select
          ...
          class="bg-white border border-sky-200 text-xs font-semibold text-slate-700 px-2 py-0.5 rounded focus:outline-none focus:border-sky-500 cursor-pointer transition-colors shadow-sm shrink-0"
        >
      ...
      >
        ...
        <div class="space-y-2">
          ...
          <p class="text-sm text-slate-500 font-medium leading-relaxed">
            You do not have permission to view the Balance Sheet Report. Please contact your system
        ...
        <div class="pt-2">
          <RouterLink
            to="/dashboard"
            class="inline-flex items-center gap-2 px-5 py-2.5 bg-[#107c41] hover:bg-[#0b592e] text-white text-sm font-bold rounded-xl shadow-md active:scale-95 transition-all"
          >
            Back to Dashboard
          </RouterLink>
        </div>
    ...
    >
      <div
        class="w-10 h-10 border-4 border-slate-100 border-t-[#107c41] rounded-full animate-spin"
      ></div>
      <p class="text-[#107c41] font-medium animate-pulse text-sm mt-4">
    ...
    <div
      v-else-if="displayFormat === '1' && multiReports.current"
      class="p-6 w-full flex flex-col flex-1 min-h-0 overflow-hidden"
    >
      <div class="w-full flex-1 min-h-0 overflow-auto custom-scrollbar">
        <div
          class="spreadsheet-table bg-white border border-[#cbd5e1] rounded shadow-sm inline-block min-w-max mx-auto relative select-none"
        >
          <table class="border-collapse text-[11px] font-sans text-slate-800 w-full">
            <!-- Column letters headers -->
            <thead>
              <tr
                class="bg-[#f1f5f9] text-slate-500 font-mono text-[9px] font-semibold border-b border-[#cbd5e1]"
              >
                <th
                  class="border-r border-[#cbd5e1] bg-[#e2e8f0] w-9 text-center p-0 font-sans"
                ></th>
                <th class="border-r border-[#cbd5e1] text-center w-60 py-0.5">A</th>
                <th class="border-r border-[#cbd5e1] text-center w-28 py-0.5">B</th>
                <th v-if="showBudget" class="border-r border-[#cbd5e1] text-center w-28 py-0.5">
                  C
                </th>
            ...
            <thead>
              <tr
                class="bg-[#f8fafc] font-black text-slate-700 border-b border-[#cbd5e1] tracking-wider text-[10px] uppercase shadow-sm"
              >
                <th
                  class="excel-col-letter-cell bg-[#f1f5f9] border-r border-[#cbd5e1] w-9 p-0"
                ></th>
                <!-- Assets Title -->
                <th class="border-r border-[#cbd5e1] px-3.5 py-2 text-left">Assets</th>
                <th class="border-r border-[#cbd5e1] px-3 py-2 text-right">Balance (IDR)</th>
                <th v-if="showBudget" class="border-r border-[#cbd5e1] px-3 py-2 text-right">
                  Budget (IDR)
                </th>
                ...
                <!-- Liab / Eq Title -->
                <th class="border-r border-[#cbd5e1] px-3.5 py-2 text-left">Liabilities</th>
                <th class="border-r border-[#cbd5e1] px-3 py-2 text-right">Balance (IDR)</th>
            ...
            <tbody>
              ...
              >
                ...
                <template v-else>
                  ...
                  <td
                    v-if="showBudget"
                    class="border-r border-[#cbd5e1] px-3 py-1 bg-slate-50/30"
                  ></td>
                </template>
                ...
                <template v-if="row.right">
                  ...
                  <template v-if="row.right.type === 'section'">
                    ...
                    <td
                      v-if="showBudget"
                      class="border-r border-[#cbd5e1] bg-slate-100/40 py-1"
                    ></td>
                  </template>
                  ...
                  <template v-else-if="row.right.type === 'group'">
                    <td
                      @click="
                        row.right.isNetIncome ? null : openModal(row.right.name, row.right.items)
                      "
                      class="border-r border-[#cbd5e1] px-4 py-1 font-semibold text-slate-700 tracking-tight hover:text-[#107c41] cursor-pointer"
                      :class="row.right.isNetIncome ? 'text-green-600 italic font-bold' : ''"
                    >
                      {{ row.right.name }}
                    </td>
                    <td
                      class="border-r border-[#cbd5e1] px-3 py-1 text-right font-bold"
                      :class="
                        row.right.isNetIncome
                          ? isNeg(row.right.total)
                            ? 'text-red-600'
                            : 'text-green-600'
                          : isNeg(row.right.total)
                            ? 'text-red-600'
              ...
              <!-- BOTTOM DOUBLE-UNDERLINE GRAND TOTAL ROW -->
              <tr class="border-b border-[#cbd5e1] bg-green-50/20">
                <td
                ...
                <td
                  ...
                  :class="
                    isNeg(multiReports.current.structure.Assets.total)
                      ? 'text-red-600'
                      : 'text-black'
                  "
                >
                  {{ formatMoney(multiReports.current.structure.Assets.total) }}
                </td>
                <td
                  v-if="showBudget"
                  class="border-r border-[#cbd5e1] px-3 py-2 text-right font-black text-[12px] border-t-[2px] border-t-slate-400 border-b-[4px] border-b-double border-b-slate-400 bg-green-50/10"
                  :class="isNeg(format1BudgetTotals.assetsTotal) ? 'text-red-600' : 'text-black'"
                >
                ...
                <td
                  ...
                  :class="
                    isNeg(
                      multiReports.current.structure.Liabilities.total +
                        multiReports.current.structure.Equity.total +
                        multiReports.current.netIncome,
                    )
                      ? 'text-red-600'
                      : 'text-black'
                  "
    ...
    <div
      v-else-if="['2', '3', '4'].includes(displayFormat) && matrixState"
      class="p-6 w-full flex flex-col flex-1 min-h-0 overflow-hidden"
    >
      <div class="w-full flex-1 min-h-0 overflow-auto custom-scrollbar">
        <div
        ...
        >
          <table class="border-collapse text-[11px] font-sans text-slate-800 w-full">
            ...
            <thead>
              ...
              >
                <th
                  class="border-r border-[#cbd5e1] bg-[#e2e8f0] w-9 min-w-[36px] max-w-[36px] text-center p-0 font-sans sticky left-0 z-30"
                ></th>
                <th
                  class="border-r border-[#cbd5e1] text-center w-60 min-w-[240px] max-w-[240px] py-0.5 sticky left-[36px] z-30 bg-[#f1f5f9]"
                >
                  A
                </th>
            ...
            <thead>
              ...
              >
                <th
                  class="excel-col-letter-cell bg-[#f1f5f9] border-r border-[#cbd5e1] w-9 min-w-[36px] max-w-[36px] p-0 sticky left-0 z-30"
                ></th>
                <th
                ...
                <template v-for="(header, index) in matrixState.headers" :key="index">
                  ...
                  >
                    {{ header.label }}
                    <span class="text-slate-500 block text-[8px] font-bold mt-0.5 normal-case">{{
                      header.sub
                    }}</span>
                  </th>
                </template>
              ...
              >
                ...
                <th
                  class="excel-col-letter-cell bg-[#f1f5f9] border-r border-[#cbd5e1] w-9 min-w-[36px] max-w-[36px] p-0 sticky left-0 z-30"
                ></th>
                <!-- Account column is covered by rowspan="2" from the top row, so we do not put an Account cell here -->
                <template v-for="(header, index) in matrixState.headers" :key="index">
                  <th
                    class="border-r border-[#cbd5e1] text-center py-1 bg-slate-50/20 text-[#0f2240] font-black w-28"
                  >
                    Actual
                  </th>
                  <th
                    class="border-r border-[#cbd5e1] text-center py-1 bg-slate-50/20 text-slate-500 font-bold w-28"
                  >
                    Budget
                  </th>
                </template>
            ...
            <tbody>
              ...
              >
                ...
                <!-- Case C: Net income (Earnings) inside Equity -->
                <template v-else-if="row.type === 'earnings'">
                  <td
                    class="text-green-600 font-bold italic px-4 py-1 border-r border-[#cbd5e1] sticky left-[36px] z-20 bg-white min-w-[240px] max-w-[240px]"
                  >
                    {{ row.name }}
                  </td>

                  <template v-for="(netIncome, index) in row.totals" :key="index">
                    <!-- Actual -->
                    <td
                      class="border-r border-[#cbd5e1] text-right py-1 px-3 font-bold bg-green-50/10"
                      :class="isNeg(netIncome) ? 'text-red-600' : 'text-green-600'"
                    >
                    ...
                    <td
                      v-if="showBudget"
                      class="border-r border-[#cbd5e1] text-right bg-slate-50/50 py-1 px-3 font-semibold text-blue-600"
                    >
                      {{ formatMoney(row.budgets[index]) }}
                    </td>
                  </template>
                ...
                <template v-else-if="row.type === 'subtotal'">
                  ...
                  <template v-for="(total, index) in row.totals" :key="index">
                    ...
                    <td
                      ...
                      :class="
                        isNeg(
                          row.section === 'Equity' ? total + matrixState.netIncomes[index] : total,
                        )
                          ? 'text-red-600'
                          : 'text-black'
                      "
                    ...
                    >
                      {{
                        formatMoney(
                          row.section === 'Equity'
                            ? row.budgetTotals[index] + matrixState.netIncomeBudgets[index]
                            : row.budgetTotals[index],
                        )
                      }}
                    </td>
                  </template>
              ...
              <tr class="border-b border-[#cbd5e1] bg-green-50/20 font-black">
                ...
                <template v-for="(header, index) in matrixState.headers" :key="index">
                  ...
                  <td
                    ...
                    :class="
                      isNeg(
                        matrixState.data['Liabilities'].totals[index] +
                          matrixState.data['Equity'].totals[index] +
                          matrixState.netIncomes[index],
                      )
                        ? 'text-red-600'
                        : 'text-black'
                    "
                  ...
                  >
                    {{
                      formatMoney(
                        matrixState.data['Liabilities'].budgetTotals[index] +
                          matrixState.data['Equity'].budgetTotals[index] +
                          matrixState.netIncomeBudgets[index],
                      )
                    }}
                  </td>
                </template>
    ...
    <Teleport to="body">
      <transition name="fade">
        ...
        >
          ...
          <div
            class="bg-white/95 backdrop-blur-md w-full max-w-5xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-slide-up border border-[#cbd5e1]"
          >
            <div
              class="flex justify-between items-center px-6 py-4 bg-[#f8fafc] border-b border-slate-200 shrink-0 relative z-10 select-none"
            >
              <div>
                <h3 class="text-base md:text-lg font-black text-slate-900 tracking-tight">
                  {{ activeGroupTitle }}
                </h3>
                <p
                  class="text-xs text-[#107c41] mt-0.5 font-bold uppercase tracking-widest font-mono"
                >
                  Ledgers breakdown sheet
                </p>
              </div>
              <button
                @click="closeModal"
                class="text-slate-400 hover:text-rose-500 transition-colors bg-white hover:bg-rose-50 p-2 rounded-lg border border-slate-200 shadow-sm outline-none"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-5 w-5"
            ...
            <div class="overflow-auto grow p-0 custom-scrollbar relative z-10">
              <table
                class="w-full text-left border-collapse relative z-10 text-[11px] font-sans text-slate-800"
              >
                <thead
                  class="sticky top-0 bg-[#f8fafc]/90 backdrop-blur-sm z-10 border-b border-[#cbd5e1] shadow-sm font-black uppercase text-slate-600 tracking-wider"
                >
                  <tr>
                    <th class="px-6 py-2.5 min-w-[320px]">Account No & Name</th>
                    <template v-if="activeGroupHeaders.length === 0">
                      <th class="px-6 py-2.5 text-right whitespace-nowrap min-w-48">
                        Actual Balance
                      </th>
                      <th
                        v-if="showBudget"
                        class="px-6 py-2.5 text-right whitespace-nowrap min-w-48 border-l border-slate-100 bg-slate-50/50"
                      >
                        Budget Balance
                      </th>
                    </template>
                    <template v-else v-for="(header, index) in activeGroupHeaders" :key="index">
                      <th
                        class="px-6 py-2.5 text-right whitespace-nowrap min-w-48 border-l border-slate-100"
                      >
                        {{ header.label }}
                        <span
                          class="text-slate-400 block text-[9px] font-medium tracking-normal mt-0.5 normal-case"
                          >{{ header.sub }} <span v-if="showBudget">(ACT)</span></span
                        >
                      </th>
                      <th
                        v-if="showBudget"
                        class="px-6 py-2.5 text-right whitespace-nowrap min-w-48 border-l border-slate-200/60 bg-slate-50/50"
                      >
                        {{ header.label }}
                        <span
                          class="text-[#107c41] block text-[9px] font-semibold tracking-normal mt-0.5 normal-case"
                          >{{ header.sub }} (BGT)</span
                        >
                      </th>
                    </template>
                ...
                <tbody class="divide-y divide-slate-200 bg-white">
                  ...
                  >
                    <td
                      class="px-6 py-2 text-slate-700 tracking-tight whitespace-normal break-words"
                    >
                      <div class="flex items-start">
                        <span class="font-mono text-slate-400 mr-3 shrink-0 font-bold">{{
                          item.no
                        }}</span>
                    ...
                    <template v-if="item.balance !== undefined">
                      <td class="px-6 py-2 border-l border-slate-100 text-right">
                        <strong
                          :class="[
                            'font-bold font-mono',
                            isNeg(item.balance) ? 'text-red-500' : 'text-black',
                          ]"
                          >{{ formatMoney(item.balance) }}</strong
                        >
                      </td>
                      <td
                        v-if="showBudget"
                        class="px-6 py-2 border-l border-slate-100 bg-slate-50/30 text-right"
                      >
                        <strong class="font-medium font-mono text-blue-600">{{
                          formatMoney(item.end_budget || 0)
                        }}</strong>
                      </td>
                    </template>
                    <template v-else>
                      <template v-for="(balance, index) in item.balances" :key="index">
                        <td class="px-6 py-2 border-l border-slate-100 text-right">
                          <strong
                            :class="[
                              'font-bold font-mono',
                              isNeg(balance) ? 'text-red-500' : 'text-black',
                            ]"
                            >{{ formatMoney(balance) }}</strong
                          >
                        </td>
                        <td
                          v-if="showBudget"
                        ...
                        >
                          <strong class="font-medium font-mono text-blue-600">{{
                            formatMoney(item.budgets[index])
                          }}</strong>
                        </td>
                      </template>
                    </template>
            ...
            <div
              class="px-8 py-3 bg-[#f8fafc] border-t border-slate-200 flex justify-end shrink-0 select-none"
            >
              <button
                @click="closeModal"
                class="bg-[#107c41] hover:bg-[#0b592e] text-white px-5 py-1.5 rounded text-xs font-bold transition-colors shadow-sm outline-none"
              >
                Close Breakdown
              </button>
            </div>
