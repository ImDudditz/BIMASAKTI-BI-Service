...
export const useReportFilterStore = defineStore('reportFilters', () => {
  ...
  const companyName = ref(
    sessionStorage.getItem('reportCompanyName') || 'PT. Agung Sedayu Propertindo',
  )
  ...
  const fetchFilters = async () => {
    const isCompanyChanged =
      lastFetchedCompanyId.value && lastFetchedCompanyId.value !== companyId.value
    if (!isCompanyChanged && availableYears.value.length > 0 && availablePeriods.value.length > 0)
      return
    try {
      ...
      if (res.data.status === 'success') {
        ...
        if (res.data.years.length > 0) {
          if (
            isCompanyChanged ||
            !selectedYear.value ||
            !res.data.years.includes(selectedYear.value)
          ) {
        ...
        if (res.data.periods.length > 0) {
          if (
            ...
            !res.data.periods.includes(selectedPeriod.value)
          ) {
            selectedPeriod.value =
              res.data.latest_period || res.data.periods[res.data.periods.length - 1]
          }
