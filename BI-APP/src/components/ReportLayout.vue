...
<template>
  ...
  >
    <header
      class="shrink-0 bg-[#ebf3fd] border-b border-[#a8cbf3] shadow-sm z-[100] w-full relative"
    >
      <div
        class="max-w-screen-2xl mx-auto px-3 sm:px-4 py-1.5 sm:py-2 flex flex-col md:flex-row md:items-center justify-between gap-2"
      >
        <!-- LEFT SIDE: Title & Subtitle with mobile hamburger toggle -->
        <div
          class="flex items-center justify-between md:justify-start gap-3 w-full md:w-auto shrink-0"
        >
          <div class="flex items-center gap-2.5">
            ...
            <div class="flex flex-col text-left shrink-0">
              <h2
                class="text-xs sm:text-[13px] md:text-sm font-black text-[#0f2240] tracking-tight leading-tight whitespace-nowrap font-['Outfit'] uppercase"
              >
                {{ title }}
              </h2>
              <p
                v-if="subtitle"
                class="text-[7.5px] sm:text-[8px] font-black text-sky-500 uppercase tracking-widest mt-0.5 leading-none"
              >
                {{ subtitle }}
              </p>
            </div>
          ...
          <div
            v-if="['/balance-sheet', '/income-statement'].includes(router.currentRoute.value.path)"
            class="ml-2"
          >
            <select
        ...
        <div
          class="flex-grow flex items-center justify-start md:pl-6 min-w-0 w-full md:w-auto overflow-x-auto custom-scrollbar whitespace-nowrap py-1"
        >
        ...
        <div
          class="flex items-center justify-end gap-3 shrink-0 w-full md:w-auto mt-1 md:mt-0"
          v-if="$slots.actions"
        >
