...
const fetchData = async () => {
  ...
  try {
    ...
    error.value =
      ...
      'Unable to load maintenance dashboard data. Please verify your authentication or try again.'
  } finally {
...
<template>
  <div
    class="relative bg-white/90 backdrop-blur-md border border-slate-100 rounded-2xl shadow-sm p-4 overflow-hidden flex flex-col h-[330px] hover:shadow-md hover:border-slate-200/80 transition-all duration-300"
  >
    <!-- Header Block -->
    <div class="flex items-center justify-between mb-2.5 shrink-0">
      <div>
        ...
        <p class="text-[9px] sm:text-[10px] font-medium text-slate-400 mt-0.5">
          Active tickets and uptime reliability
        </p>
      </div>

      <!-- System Normal Status Badge -->
      <span
        class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-sky-50 text-sky-700 shrink-0"
      >
    ...
    <div
      v-if="isLoading"
      class="flex-1 flex flex-col items-center justify-center py-4 min-h-0 w-full"
    >
      <div
        class="w-8 h-8 border-3 border-slate-100 border-t-amber-500 rounded-full animate-spin"
      ></div>
      <p class="text-amber-600/90 font-semibold animate-pulse text-xs mt-3 tracking-wide">
    ...
    <div
      v-else-if="error"
      class="flex-1 flex flex-col items-center justify-center py-4 px-3 text-center min-h-0 w-full"
    >
      ...
      <p class="text-slate-500 text-[10px] mt-0.5 max-w-[200px] leading-relaxed truncate">
        {{ error }}
      </p>
      <button
        @click="fetchData"
        class="mt-2 px-3 py-1 bg-amber-500 hover:bg-amber-600 text-white text-[10px] font-bold rounded shadow transition-all duration-200"
      >
        Try Again
      </button>
    </div>
    ...
    <div v-else class="flex-1 min-h-0 flex flex-row gap-3 items-center">
      ...
      <div class="flex flex-col gap-2 w-[125px] sm:w-[135px] shrink-0">
        <!-- Open Tickets Card -->
        <div
          class="p-2 sm:p-2.5 rounded-xl bg-gradient-to-br from-slate-50 to-slate-100/50 border border-slate-100/60 flex flex-col justify-between h-[105px]"
        >
        ...
        <div
          class="p-2 sm:p-2.5 rounded-xl bg-gradient-to-br from-slate-50 to-slate-100/50 border border-slate-100/60 flex flex-col justify-between h-[105px]"
          :class="{ 'from-rose-50/40 border-rose-100': criticalAlerts > 0 }"
        >
          <span class="text-[9px] font-bold text-slate-500 uppercase tracking-wider"
          ...
          <div class="flex items-baseline gap-1 mt-1">
            <span
              ...
              :class="criticalAlerts > 0 ? 'text-rose-600' : 'text-slate-800'"
              >{{ criticalAlerts }}</span
            >
            <span
              class="text-[8px] sm:text-[9px] font-bold"
              :class="criticalAlerts > 0 ? 'text-rose-500 animate-pulse' : 'text-slate-500'"
              >warnings</span
            >
          </div>
        </div>
