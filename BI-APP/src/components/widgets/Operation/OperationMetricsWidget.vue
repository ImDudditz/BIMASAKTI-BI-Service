...
const fetchData = async () => {
  ...
  try {
    ...
    error.value =
      ...
      'Unable to load operational dashboard data. Please verify your authentication or try again.'
  } finally {
...
<template>
  ...
  >
    ...
    <div class="flex items-center justify-between mb-2.5 shrink-0">
      <div>
        ...
        <p class="text-[9px] sm:text-[10px] font-medium text-slate-400 mt-0.5">
          Active tenants and traffic statistics
        </p>
      </div>

      <!-- Live Indicator Badge -->
      <span
        class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-emerald-50 text-emerald-700 shrink-0"
      >
    ...
    >
      <div
        class="w-8 h-8 border-3 border-slate-100 border-t-indigo-600 rounded-full animate-spin"
      ></div>
      <p class="text-indigo-600/90 font-semibold animate-pulse text-xs mt-3 tracking-wide">
    ...
    >
      ...
      <button
        @click="fetchData"
        class="mt-2 px-3 py-1 bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] font-bold rounded shadow transition-all duration-200"
      >
        Try Again
      </button>
    </div>
