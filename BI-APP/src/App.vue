...
watch(
  ...
  (isAuth) => {
    if (
      isAuth &&
      (router.currentRoute.value.path === '/' || router.currentRoute.value.path === '/login')
    ) {
      router.push('/overview')
    }
  },
...
watch(
  ...
  ([path, name, id]) => {
    if (path === '/login') {
      document.title = 'BI_SERVICE - Login'
    } else if (name) {
...
<template>
  ...
  <div
    class="h-screen w-screen flex flex-col font-['Plus_Jakarta_Sans',sans-serif] text-[#1c2e4a] overflow-hidden relative bg-[#f1f6fa] select-none selection:bg-sky-500/30"
  >
    <!-- Decorative Royale Light Blue Metallic Ribbon at Top -->
    <div
      class="w-full h-1.5 bg-gradient-to-r from-sky-400 via-blue-500 to-sky-300 relative z-30 shrink-0"
    ></div>

    <!-- Beautiful Architectural Skyscraper Grayscale Background with 75% Transparency (opacity-20) -->
    <div
      v-if="isAuthPage"
      class="absolute inset-x-0 top-0 h-[750px] lg:h-[900px] pointer-events-none opacity-20 overflow-hidden z-0 flex items-end justify-center select-none"
    >
    ...
    <div
      v-if="isAuthPage"
      class="flex-1 flex gap-3 lg:gap-4 p-2 sm:p-3 lg:p-4 overflow-hidden h-full w-full relative z-10"
    >
      ...
      >
        ...
        <div
          class="bg-white/80 rounded-xl border border-[#95bfe9] p-3 flex flex-col items-center justify-center text-center shadow-sm shrink-0 gap-2 mb-4"
        >
          ...
          <div class="flex flex-col items-center min-w-0 max-w-full">
            <h1
              class="text-[10px] font-black text-[#0c3c8c] tracking-widest uppercase leading-snug break-words whitespace-normal px-1 font-['Outfit']"
              :title="companyName"
            >
              {{ companyName }}
            </h1>
        ...
        <nav class="flex-grow overflow-y-auto pr-1 space-y-3 custom-scrollbar">
          ...
          <div class="bg-white/70 rounded-xl overflow-hidden border border-[#95bfe9] shadow-sm">
            <div
              class="px-3.5 py-2 flex items-center justify-between bg-gradient-to-r from-[#215dc6] to-[#6da3f6] text-white text-[10px] font-black uppercase tracking-wider font-['Outfit']"
            >
          ...
          <div class="bg-white/70 rounded-xl overflow-hidden border border-[#95bfe9] shadow-sm">
            ...
            >
              ...
              <span
                ...
                :class="{ 'rotate-180': expandedMenus.financials }"
                >▼</span
              >
            </button>
            ...
            >
              ...
              <RouterLink
                ...
                class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold border transition-all duration-200 outline-none"
                :class="
                  ['/balance-sheet', '/income-statement'].includes(router.currentRoute.value.path)
                    ? 'bg-[#3b82f6]/15 text-[#0c3c8c] border-[#3b82f6]/30'
                    : 'text-slate-600 border-transparent hover:bg-[#3b82f6]/5 hover:text-[#0c3c8c]'
                "
              >
              ...
              <RouterLink
                ...
                class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold border transition-all duration-200 outline-none"
                :class="
                  router.currentRoute.value.path === '/print'
                    ? 'bg-[#3b82f6]/15 text-[#0c3c8c] border-[#3b82f6]/30'
                    : 'text-slate-600 border-transparent hover:bg-[#3b82f6]/5 hover:text-[#0c3c8c]'
                "
              >
          ...
          <div class="bg-white/70 rounded-xl overflow-hidden border border-[#95bfe9] shadow-sm">
            ...
            >
              ...
              <span
                class="text-[9px] transition-transform duration-300"
                :class="{ 'rotate-180': expandedMenus.tenancy }"
                >▼</span
              >
            </button>
            ...
            >
              <RouterLink
                ...
                class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold border transition-all duration-200 outline-none"
                :class="
                  router.currentRoute.value.path === '/dashboard/operation'
                    ? 'bg-[#3b82f6]/15 text-[#0c3c8c] border-[#3b82f6]/30'
                    : 'text-slate-600 border-transparent hover:bg-[#3b82f6]/5 hover:text-[#0c3c8c]'
                "
              >
              ...
              <RouterLink
                ...
                class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold border transition-all duration-200 outline-none"
                :class="
                  router.currentRoute.value.path === '/tenancy-report'
                    ? 'bg-[#3b82f6]/15 text-[#0c3c8c] border-[#3b82f6]/30'
                    : 'text-slate-600 border-transparent hover:bg-[#3b82f6]/5 hover:text-[#0c3c8c]'
                "
              >
          ...
          <div class="bg-white/70 rounded-xl overflow-hidden border border-[#95bfe9] shadow-sm">
            ...
            >
              ...
              <span
                class="text-[9px] transition-transform duration-300"
                :class="{ 'rotate-180': expandedMenus.services }"
                >▼</span
              >
            </button>
            ...
            >
              <RouterLink
                ...
                class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold border transition-all duration-200 outline-none"
                :class="
                  router.currentRoute.value.path === '/dashboard/maintenance'
                    ? 'bg-[#3b82f6]/15 text-[#0c3c8c] border-[#3b82f6]/30'
                    : 'text-slate-600 border-transparent hover:bg-[#3b82f6]/5 hover:text-[#0c3c8c]'
                "
              >
              ...
              <RouterLink
                ...
                class="flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold border transition-all duration-200 outline-none"
                :class="
                  router.currentRoute.value.path === '/services-report'
                    ? 'bg-[#3b82f6]/15 text-[#0c3c8c] border-[#3b82f6]/30'
                    : 'text-slate-600 border-transparent hover:bg-[#3b82f6]/5 hover:text-[#0c3c8c]'
                "
              >
        ...
        <div
          class="flex items-center justify-center gap-1.5 border-t border-[#95bfe9]/50 pt-2 mt-auto"
        >
          <div
      ...
      <div class="flex-1 flex flex-col overflow-hidden h-full min-w-0">
        ...
        <main
          class="flex-1 bg-[#f3f7fd] rounded-2xl border border-[#1f62d4] shadow-[0_20px_50px_-12px_rgba(31,98,212,0.35)] overflow-hidden relative flex flex-col min-h-0"
        >
          <!-- Authentic Windows XP Royale Energy Blue Title Bar -->
          <div
            class="flex items-center justify-between px-3 sm:px-4 py-1.5 sm:py-2 border-t border-white/40 bg-gradient-to-b from-[#629ef7] via-[#357ae5] to-[#1b56ca] text-white relative z-20 shrink-0 shadow-[inset_0_1px_0_rgba(255,255,255,0.4)] select-none"
          >
            <div class="flex items-center gap-2 min-w-0">
              ...
              <svg
                ...
                stroke-width="2.5"
                stroke="currentColor"
                class="w-4 h-4 text-sky-100 drop-shadow-[0_1px_1px_rgba(0,0,0,0.5)] shrink-0"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
              ...
              <!-- Dynamic Title Text -->
              <span
                class="text-[10px] sm:text-xs font-black tracking-wide text-white drop-shadow-[0_1px_2px_rgba(0,0,0,0.8)] font-['Outfit'] uppercase truncate max-w-[120px] xs:max-w-[180px] sm:max-w-none"
              >
            ...
            <div class="flex items-center gap-4">
              <!-- Notification Bell -->
              <button
                class="w-8 h-8 rounded-full bg-white/60 border border-slate-200/40 hover:bg-white hover:border-slate-200 flex items-center justify-center shadow-sm relative transition-all active:scale-95 group"
              >
                <span class="text-sm group-hover:rotate-12 transition-transform">🔔</span>
                <span
                  class="absolute top-1 right-1 w-1.5 h-1.5 rounded-full bg-[#3c56d6] ring-1 ring-white"
                ></span>
              </button>
              ...
              <div class="flex items-center gap-2.5 pl-1" v-if="authStore.user">
                <!-- Avatar Circle -->
                <div
                  class="w-7 h-7 rounded-full bg-[#3c56d6]/20 border border-[#3c56d6]/30 text-white flex items-center justify-center text-[11px] font-black uppercase shadow-inner"
                >
                ...
                <div class="flex flex-col text-left hidden sm:flex leading-none">
                  ...
                  <span
                    class="text-[8px] font-bold text-sky-200 leading-none mt-0.5 uppercase tracking-wider"
                  >
              ...
              <button
                @click="handleLogout"
                class="flex items-center gap-1.5 text-[11px] font-bold text-slate-500 hover:text-rose-600 hover:bg-rose-50/50 border border-transparent hover:border-rose-200/50 px-2.5 py-1.5 rounded-xl transition-all duration-200 active:scale-95 group shrink-0 bg-white/60 shadow-sm"
              >
                <svg
                  class="w-3.5 h-3.5 text-slate-400 group-hover:text-rose-500 transition-colors"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                  xmlns="http://www.w3.org/2000/svg"
    ...
    <div
      v-else
      class="flex-1 w-full h-full flex flex-col overflow-hidden relative z-10 bg-[#f1f6fa]"
    >
      <RouterView class="grow overflow-hidden flex flex-col w-full relative z-10" />
    </div>
    ...
    <Teleport to="body">
      <div v-if="isOpen" class="fixed inset-0 z-[999] flex items-center justify-center p-4">
        ...
        <div
          class="relative bg-[#f3f7fd] rounded-xl shadow-2xl max-w-md w-full overflow-hidden border border-[#1f62d4] animate-slide-up flex flex-col font-['Plus_Jakarta_Sans',sans-serif]"
        >
          <!-- Dialog Title Bar -->
          <div
            class="flex items-center justify-between px-3 py-1.5 bg-gradient-to-b from-[#629ef7] via-[#357ae5] to-[#1b56ca] text-white select-none"
          >
            <span class="text-[10px] font-black uppercase tracking-wider font-['Outfit']">{{
              title || 'Bimasakti BI System Message'
            }}</span>
            <button
              @click="modalStore.closeModal"
              class="w-4 h-4 rounded-full flex items-center justify-center border border-[#9b1708] bg-gradient-to-b from-[#fbae97] via-[#f3664d] to-[#bf2613] hover:from-[#ffbfad] hover:to-[#df301b] transition-all"
            >
          ...
          <div class="p-5 flex gap-4">
            <div
              class="text-3xl bg-white border border-[#95bfe9] p-3 rounded-lg shadow-sm shrink-0 flex items-center justify-center"
            >
              {{ icon }}
            </div>
            <div class="w-full flex flex-col text-left">
              <h3
                class="text-sm font-black text-[#0f2240] tracking-tight leading-tight font-['Outfit']"
              >
                {{ title }}
              </h3>
          ...
          <!-- Dialog Actions Bar -->
          <div
            class="bg-[#ebf3fd] px-4 py-2.5 flex items-center justify-end gap-2 border-t border-[#a8cbf3]"
          >
            <button
              v-if="type !== 'alert'"
              @click="modalStore.closeModal"
