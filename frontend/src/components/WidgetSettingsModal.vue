...
const loadWidgets = async () => {
  try {
    ...
    localWidgets.value = masterWidgets.map(mw => {
...
<template>
  <div v-if="show" class="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm transition-opacity">
    <div class="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col border border-slate-200">
      ...
      <div class="p-6 flex flex-col gap-4 overflow-y-auto max-h-[60vh] custom-scrollbar">
        <p class="text-sm text-slate-500 mb-2">Select which widgets you want to display on your executive dashboard.</p>
        
        <div v-for="widget in localWidgets" :key="widget.widget_key" class="flex items-center justify-between p-4 rounded-lg border border-slate-200 hover:border-sky-300 hover:bg-sky-50/50 transition-colors bg-white">
      ...
      <div class="px-6 py-4 border-t border-slate-100 bg-slate-50 flex justify-end gap-3">
        <button @click="emit('close')" class="px-4 py-2 text-sm font-medium text-slate-600 hover:bg-slate-200 bg-slate-100 rounded transition-colors">
          Cancel
        </button>
        <button @click="saveLayout" :disabled="isSaving" class="px-5 py-2 text-sm font-medium text-white bg-sky-600 hover:bg-sky-700 rounded shadow-sm disabled:opacity-70 flex items-center transition-colors">
