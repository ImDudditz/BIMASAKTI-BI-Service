<template>
  <div class="relative bg-white/90 backdrop-blur-md border border-slate-100 rounded-2xl shadow-sm p-6 overflow-hidden flex flex-col h-[380px] hover:shadow-md hover:border-slate-200/80 transition-all duration-300">
    ...
    <div class="flex items-center justify-between mb-4">
      <div>
        <h4 class="text-sm font-bold text-slate-800 tracking-tight">Revenue vs Budget</h4>
        <p class="text-[11px] font-medium text-slate-400">Monthly PTD revenue against budget for {{ props.selectedYear }}</p>
      </div>
    </div>
...
const chartOption = ref({
  ...
  legend: {
    data: ['Actual Revenue', 'Budget Revenue'],
    top: '0%',
    right: '0%',
    icon: 'circle',
  ...
  tooltip: {
    ...
    formatter: (params) => {
      ...
      params.forEach(param => {
        ...
        const textColor = isActual ? '#bae6fd' : '#e2e8f0'
        result += `
