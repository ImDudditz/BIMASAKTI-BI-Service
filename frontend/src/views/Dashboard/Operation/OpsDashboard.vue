    ...
    <div class="overflow-y-auto custom-scrollbar flex-1 min-h-0 w-full relative z-10">
      <div class="max-w-screen-2xl mx-auto px-2 sm:px-4 lg:px-6 py-4 flex flex-col min-h-full w-full">
          ...
          <div class="max-w-md w-full bg-white/85 backdrop-blur-md rounded-2xl shadow-xl border border-slate-200/60 p-8 space-y-6 animate-slide-up">
            ...
            <div class="space-y-2">
              ...
              <p class="text-sm text-slate-500 font-medium leading-relaxed">
                You do not have permission to view the Operations Dashboard. Please contact your system administrator to request access.
              </p>
            </div>
            <div class="pt-2">
              <RouterLink to="/dashboard" class="inline-flex items-center gap-2 px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl shadow-md hover:shadow-indigo-500/20 active:scale-95 transition-all duration-200">
                Back to Dashboard
              </RouterLink>
            </div>
          </div>
        ...
        <div v-else-if="isLoading" class="flex flex-col items-center justify-center flex-1 min-h-0 w-full py-24">
          ...
          <p class="text-indigo-600 font-medium animate-pulse text-sm mt-4">Loading operational databases...</p>
        </div>
        ...
        <div v-else-if="error" class="flex flex-col items-center justify-center flex-1 min-h-0 w-full py-20 text-center">
          ...
          <button @click="loadDashboardData" class="mt-4 px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-lg shadow-sm transition-all duration-200">
            Retry Connection
          </button>
        </div>
...
const isAuthorized = computed(() => {
  return authStore.isAdmin || authStore.userWidgets.includes('operation_metrics') || authStore.userWidgets.includes('lease_expirations')
})
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    error.value = err.response?.data?.message || 'Operational service connection timeout. Please verify network routing or backend session authorization.'
  } finally {
    isLoading.value = false
  }
}
