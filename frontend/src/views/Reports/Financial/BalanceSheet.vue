...
const matrixRows = computed(() => {
  ...
  Object.entries(matrixState.value.data).forEach(([sectionName, sectionData]) => {
    ...
    rows.push({
      ...
      section: sectionName,
      rowNum: rowIdx++,
      start: sectionName === 'Assets' ? assetsStart : (sectionName === 'Liabilities' ? liabStart : eqStart),
...
<template>
  ...
  >
    <template #controls>
      <div v-if="isAuthorized && availableYears.length > 0" class="flex items-center justify-start gap-2 w-full">
        ...
        <div class="flex items-center gap-1.5 bg-white border border-sky-200 rounded px-2.5 py-1 shadow-sm shrink-0">
          <label class="flex items-center gap-1.5 cursor-pointer text-sm font-medium text-[#107c41] whitespace-nowrap">
            <input type="checkbox" v-model="showBudget" class="w-3.5 h-3.5 text-[#107c41] rounded border-slate-300 focus:ring-[#107c41] cursor-pointer">
            Budget
          </label>
        </div>
    ...
    <div v-else-if="isLoading" class="flex flex-col items-center justify-center flex-1 min-h-0 w-full h-full">
      ...
      <p class="text-[#107c41] font-medium animate-pulse text-sm mt-4">Loading spreadsheet data...</p>
    </div>
            ...
            <tbody>
              ...
              >
                ...
                <template v-if="row.right">
                  ...
                  <template v-else-if="row.right.type === 'group'">
                    <td 
                      @click="row.right.isNetIncome ? null : openModal(row.right.name, row.right.items)"
                      class="border-r border-[#cbd5e1] px-4 py-1 font-semibold text-slate-700 tracking-tight hover:text-[#107c41] cursor-pointer"
                    ...
                    <td 
                      class="border-r border-[#cbd5e1] px-3 py-1 text-right font-bold"
                      :class="row.right.isNetIncome ? (isNeg(row.right.total) ? 'text-red-600' : 'text-green-600') : (isNeg(row.right.total) ? 'text-red-600' : 'text-black')"
                    >
              ...
              >
                ...
                <td 
                  ...
                  :class="isNeg(multiReports.current.structure.Assets.total) ? 'text-red-600' : 'text-black'"
                >
                ...
                <td 
                  class="border-r border-[#cbd5e1] px-3 py-2 text-right font-black text-[12px] border-t-[2px] border-t-slate-400 border-b-[4px] border-b-double border-b-slate-400 bg-green-50/10"
                  :class="isNeg(multiReports.current.structure.Liabilities.total + multiReports.current.structure.Equity.total + multiReports.current.netIncome) ? 'text-red-600' : 'text-black'"
                >
                  {{ formatMoney(multiReports.current.structure.Liabilities.total + multiReports.current.structure.Equity.total + multiReports.current.netIncome) }}
                </td>
                <td 
                  v-if="showBudget"
                ...
                <template v-else-if="row.type === 'subtotal'">
                  ...
                  <template v-for="(total, index) in row.totals" :key="index">
                    ...
                    <td 
                      ...
                      :class="isNeg(row.section === 'Equity' ? total + matrixState.netIncomes[index] : total) ? 'text-red-600' : 'text-black'"
                    >
                      {{ formatMoney(row.section === 'Equity' ? total + matrixState.netIncomes[index] : total) }}
                    </td>
                    <!-- Budget -->
                    <td 
                      v-if="showBudget"
                    ...
                    >
                      {{ formatMoney(row.section === 'Equity' ? row.budgetTotals[index] + matrixState.netIncomeBudgets[index] : row.budgetTotals[index]) }}
                    </td>
                  </template>
                </template>
              ...
              <tr class="border-b border-[#cbd5e1] bg-green-50/20 font-black">
                ...
                <template v-for="(header, index) in matrixState.headers" :key="index">
                  ...
                  <td 
                    ...
                    :class="isNeg(matrixState.data['Liabilities'].totals[index] + matrixState.data['Equity'].totals[index] + matrixState.netIncomes[index]) ? 'text-red-600' : 'text-black'"
                  >
                    {{ formatMoney(matrixState.data['Liabilities'].totals[index] + matrixState.data['Equity'].totals[index] + matrixState.netIncomes[index]) }}
                  </td>
                  <!-- Budget Grand Total -->
                  <td 
                    v-if="showBudget"
                  ...
                  >
                    {{ formatMoney(matrixState.data['Liabilities'].budgetTotals[index] + matrixState.data['Equity'].budgetTotals[index] + matrixState.netIncomeBudgets[index]) }}
                  </td>
                </template>
              </tr>
            </tbody>
      ...
      <div v-if="isModalOpen" class="fixed inset-0 z-[1000] flex items-center justify-center p-4 sm:p-6">
        ...
        <div class="bg-white/95 backdrop-blur-md w-full max-w-5xl rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-slide-up border border-[#cbd5e1]">
              ...
              <thead class="sticky top-0 bg-[#f8fafc]/90 backdrop-blur-sm z-10 border-b border-[#cbd5e1] shadow-sm font-black uppercase text-slate-600 tracking-wider">
                <tr>
                  ...
                  <template v-if="activeGroupHeaders.length === 0">
                    <th class="px-6 py-2.5 text-right whitespace-nowrap min-w-48">Actual Balance</th>
          ...
          <div class="px-8 py-3 bg-[#f8fafc] border-t border-slate-200 flex justify-end shrink-0 select-none">
            <button @click="closeModal" class="bg-[#107c41] hover:bg-[#0b592e] text-white px-5 py-1.5 rounded text-xs font-bold transition-colors shadow-sm outline-none">Close Breakdown</button>
          </div>
