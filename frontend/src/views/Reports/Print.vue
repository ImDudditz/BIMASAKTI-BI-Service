...
const { companyName, companyId, selectedYear, selectedPeriod, availableYears, availablePeriods, activePreset } = storeToRefs(filterStore)
...
<template>
  <ReportLayout title="Print Reports" subtitle="Configure Period & Download Report Files">
    <div class="flex-1 flex flex-col items-center justify-center p-8 max-w-screen-2xl mx-auto w-full h-full min-h-0">
      <div class="max-w-lg w-full bg-white/70 backdrop-blur-md border border-white/80 rounded-[32px] p-8 shadow-2xl space-y-6">
        
        <div class="w-16 h-16 bg-emerald-50 border border-emerald-100 rounded-full flex items-center justify-center mx-auto text-3xl text-emerald-600 shadow-inner">
          🖨️
        </div>

        <div class="text-center space-y-1">
          <h3 class="text-lg font-black text-slate-900 tracking-tight">Print Financial Statements</h3>
          <p class="text-xs text-slate-400 font-bold">Select parameters below to download comprehensive global statements</p>
        </div>

        <!-- Param Selectors -->
        <div class="bg-slate-50/50 border border-slate-200/40 rounded-2xl p-5 flex flex-col gap-4">
          <span class="text-[10px] font-black text-slate-400 uppercase tracking-wider block">Report Parameters</span>
          
          <div class="grid grid-cols-2 gap-4">
            <div class="flex flex-col gap-1.5">
              <label class="text-[11px] font-bold text-slate-500">Select Month / Period</label>
              <select v-model="selectedPeriod" class="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-black text-slate-700 outline-none cursor-pointer focus:ring-2 focus:ring-cyan-500 shadow-sm w-full">
            ...
            <div class="flex flex-col gap-1.5">
              ...
              <select v-model="selectedYear" class="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-black text-slate-700 outline-none cursor-pointer focus:ring-2 focus:ring-cyan-500 shadow-sm w-full">
