...
const loadOverviewData = async () => {
  ...
  try {
    ...
    const financialsPromise = api.get('/reports/ledger', { 
    ...
    const prevFinancialsPromise = api.get('/reports/ledger', { 
      params: { year: prevY, period: prevP, preset: activePreset.value, company_id: company_id },
      signal
    }).then(res => res.data).catch(() => null)

    // 2. Fetch operations / occupancy
    const operationsPromise = api.get('/v1/dashboard/operation/metrics', {
      params: { year: y, period: p, company_id: company_id },
      signal
    }).then(res => res.data).catch(() => null)

    // 3. Fetch maintenance breakdown
    const maintenancePromise = api.get('/v1/dashboard/maintenance/status', {
      params: { year: y, period: p, company_id: company_id },
      signal
    }).then(res => res.data).catch(() => null)

    // 4. Fetch Ticket KPIs for precise SLA resolution
    const ticketsPromise = api.get(`/engine/lmrx0800/kpi/${company_id}`, {
...
const computedTrends = computed(() => {
  ...
  const currNet = financialsData.value.current?.net_income || financialsData.value.current?.netIncome || 0
  const prevNet = financialsData.value.previous?.net_income || financialsData.value.previous?.netIncome || 0
  ...
  const costMarginGrowth = prevRevenue > 0 && currRevenue > 0 ? ((prevExpenses/prevRevenue) - (currExpenses/currRevenue)) * 100 : 0
  ...
  list.push({
    ...
    color: 'from-emerald-500/20 via-sky-500/10 to-transparent border-emerald-500/30 text-emerald-600',
  ...
  list.push({
    ...
    icon: '🏢',
    score: occupancy * 1.4,
    color: 'from-indigo-500/20 via-purple-500/10 to-transparent border-indigo-500/30 text-indigo-600',
  ...
  list.push({
    ...
    color: 'from-violet-500/20 via-indigo-500/10 to-transparent border-violet-500/30 text-violet-600',
  ...
  list.push({
    ...
    color: 'from-emerald-500/20 via-teal-500/10 to-transparent border-emerald-500/30 text-emerald-600',
  ...
  list.push({
    ...
    color: 'from-orange-500/20 via-amber-500/10 to-transparent border-orange-500/30 text-orange-600',
    ...
    <div class="overflow-y-auto custom-scrollbar flex-1 min-h-0 w-full relative z-10">
      <div class="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-6 flex flex-col gap-6 min-h-full w-full">
        ...
        <div v-if="isLoading" class="flex flex-col items-center justify-center flex-1 min-h-[450px] w-full">
          ...
          <p class="text-sky-600 font-bold animate-pulse text-sm mt-4 tracking-wide">Synthesizing Executive Ratios & Trends...</p>
        </div>

        <!-- ERROR PANEL -->
        <div v-else-if="isError" class="flex flex-col items-center justify-center flex-1 min-h-[450px] w-full text-center">
          ...
          <button @click="loadOverviewData" class="mt-5 px-6 py-2.5 bg-sky-600 hover:bg-sky-700 text-white text-xs font-black rounded-xl shadow-md transition-all active:scale-95">
        ...
        <div v-else class="flex flex-col gap-6 pb-12 w-full">
          ...
          <!-- ==================================================== -->
          <div class="relative w-full rounded-[32px] overflow-hidden shadow-2xl border border-white/10 shrink-0 select-none">
            ...
            <transition name="slide-fade" mode="out-in">
              ...
              >
                ...
                <div class="absolute -top-24 -left-24 w-80 h-80 rounded-full bg-sky-500/10 blur-[120px] animate-pulse"></div>
                <div class="absolute -bottom-24 -right-24 w-80 h-80 rounded-full bg-indigo-500/10 blur-[120px]"></div>
              </div>
            </transition>
            ...
            <div class="relative z-10 px-8 py-10 md:py-12 flex flex-col md:flex-row gap-8 items-stretch justify-between min-h-[440px] text-white">
              ...
              <div class="flex-1 flex flex-col justify-between gap-6">
                ...
                <div class="flex items-center gap-3">
                  <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-black bg-white/10 backdrop-blur-md border border-white/15 uppercase tracking-wider text-sky-300">
                    <span class="w-1.5 h-1.5 rounded-full bg-sky-400 animate-pulse"></span>
                    {{ topThreeOverall[activeSlideIndex]?.badge }}
                  </span>
                  
                  <!-- Rank indicator -->
                  <span class="text-xs font-black text-white/50 uppercase font-mono tracking-widest">
                ...
                <div class="space-y-4">
                  ...
                  <div class="flex items-center gap-4">
                    <div class="w-14 h-14 rounded-2xl bg-white/10 backdrop-blur-md flex items-center justify-center text-3xl border border-white/20 shadow-inner shrink-0 transform hover:rotate-6 transition-all duration-300">
                ...
                <div class="space-y-3 pt-2">
                  <h4 class="text-xs font-black text-sky-300 uppercase tracking-widest">Recommended Strategic Actions</h4>
                  <ul class="space-y-2">
                    <li 
              ...
              <!-- Divider (MD screens) -->
              <div class="hidden md:block w-px bg-gradient-to-b from-white/20 via-white/10 to-transparent self-stretch mx-2"></div>
              ...
              <div class="flex-1 flex flex-col justify-between gap-6">
                ...
                <div class="flex flex-col gap-2">
                  <span class="text-[10px] font-black text-white/50 uppercase tracking-wider">Primary Drivers</span>
          ...
          <div>
            ...
            <div class="mb-5 flex flex-col md:flex-row md:items-end justify-between gap-3">
              <div>
                <h3 class="text-xl font-extrabold text-slate-800 tracking-tight font-['Outfit']">Summary Trends Dashboard</h3>
                <p class="text-xs font-bold text-slate-500 mt-0.5">Unified overview showcasing key performance indices inside each system layer</p>
              </div>
              <div class="text-[10px] font-black text-sky-600 bg-sky-100/50 border border-sky-200/50 rounded-lg px-2.5 py-1 uppercase tracking-widest font-mono shrink-0">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full items-start">
              ...
              <!-- ----------------------------------------- -->
              <div class="group relative bg-white/80 backdrop-blur-md rounded-3xl p-5 border border-slate-100 hover:border-cyan-300 shadow-sm transition-all duration-300 hover:shadow-lg flex flex-col gap-5">
                
                <!-- Module Header -->
                <div class="flex items-center justify-between pb-3 border-b border-slate-100 shrink-0">
                  <div class="flex items-center gap-2.5">
                    <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-cyan-400 to-sky-500 text-white flex items-center justify-center shadow-md">
                      💳
                    </div>
                    <div>
                      <h4 class="text-sm font-black text-slate-800 uppercase tracking-wider font-['Outfit']">Financials System</h4>
                      <p class="text-[9px] font-black text-cyan-600 uppercase tracking-widest">Revenue & Profits</p>
                    </div>
                  </div>
                  <RouterLink to="/dashboard" class="w-6 h-6 rounded-lg bg-cyan-50 flex items-center justify-center text-cyan-500 hover:bg-cyan-500 hover:text-white transition-colors duration-300" title="Inspect Financials">
                    ➜
                  </RouterLink>
                </div>
                  ...
                  >
                    <div class="flex items-start justify-between">
                      <div class="flex items-center gap-2">
                        ...
                        <span class="text-[11px] font-black text-slate-700 tracking-tight leading-tight">{{ trend.title }}</span>
                      </div>
                      <span class="text-[10px] font-black px-2 py-0.5 bg-cyan-100/40 text-cyan-700 rounded-full">
                        {{ trend.valueText }}
                      </span>
                    </div>

                    <!-- Mini custom sparkline or progress bar -->
                    <div class="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden mt-1 shadow-inner relative">
                      <!-- Dynamic Fill -->
                      <div 
                        class="h-full rounded-full bg-gradient-to-r from-cyan-400 to-sky-500"
                        :style="{ width: `${Math.min(Math.max(trend.metricValue >= 0 ? trend.metricValue * 2 : 5, 8), 100)}%` }"
                      ></div>
                    </div>

                    <!-- Short comment -->
                    <p class="text-[10.5px] font-bold text-slate-500 leading-relaxed mt-0.5">
                      {{ trend.narration.split('**')[2]?.substring(0, 100) || trend.narration.substring(0, 95) }}...
                    </p>
                  </div>
                </div>
              ...
              <!-- ----------------------------------------- -->
              <div class="group relative bg-white/80 backdrop-blur-md rounded-3xl p-5 border border-slate-100 hover:border-indigo-300 shadow-sm transition-all duration-300 hover:shadow-lg flex flex-col gap-5">
                
                <!-- Module Header -->
                <div class="flex items-center justify-between pb-3 border-b border-slate-100 shrink-0">
                  <div class="flex items-center gap-2.5">
                    <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-indigo-400 to-purple-500 text-white flex items-center justify-center shadow-md">
                      🏢
                    </div>
                    <div>
                      <h4 class="text-sm font-black text-slate-800 uppercase tracking-wider font-['Outfit']">Tenancy & Ops</h4>
                      <p class="text-[9px] font-black text-indigo-600 uppercase tracking-widest">Occupancy & Leases</p>
                    </div>
                  </div>
                  <RouterLink to="/dashboard/operation" class="w-6 h-6 rounded-lg bg-indigo-50 flex items-center justify-center text-indigo-500 hover:bg-indigo-500 hover:text-white transition-colors duration-300" title="Inspect Tenancy">
                    ➜
                  </RouterLink>
                </div>
                ...
                <div class="flex flex-col gap-4">
                  ...
                  >
                    <div class="flex items-start justify-between">
                      ...
                      <span class="text-[10px] font-black px-2 py-0.5 bg-indigo-100/40 text-indigo-700 rounded-full">
                        {{ trend.valueText }}
                      </span>
                    </div>
                    ...
                    <div class="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden mt-1 shadow-inner relative">
                      ...
                      <div 
                        ...
                        :style="{ width: `${Math.min(Math.max(trend.id === 'ten_occupancy' ? trend.metricValue : (trend.id === 'ten_traffic' ? trend.metricValue / 30 : trend.metricValue * 3), 8), 100)}%` }"
                      ></div>
                    </div>
              ...
              <!-- ----------------------------------------- -->
              <div class="group relative bg-white/80 backdrop-blur-md rounded-3xl p-5 border border-slate-100 hover:border-amber-300 shadow-sm transition-all duration-300 hover:shadow-lg flex flex-col gap-5">
                
                <!-- Module Header -->
                <div class="flex items-center justify-between pb-3 border-b border-slate-100 shrink-0">
                  <div class="flex items-center gap-2.5">
                    <div class="w-9 h-9 rounded-xl bg-gradient-to-br from-amber-400 to-orange-500 text-white flex items-center justify-center shadow-md">
                      🔧
                    </div>
                    <div>
                      <h4 class="text-sm font-black text-slate-800 uppercase tracking-wider font-['Outfit']">Services & SLA</h4>
                      <p class="text-[9px] font-black text-amber-600 uppercase tracking-widest">Uptime & Service Tickets</p>
                    </div>
                  </div>
                  <RouterLink to="/dashboard/maintenance" class="w-6 h-6 rounded-lg bg-amber-50 flex items-center justify-center text-amber-500 hover:bg-amber-500 hover:text-white transition-colors duration-300" title="Inspect Services">
                    ➜
                  </RouterLink>
                </div>
                ...
                <div class="flex flex-col gap-4">
                  ...
                  >
                    <div class="flex items-start justify-between">
                      ...
                      <span class="text-[10px] font-black px-2 py-0.5 bg-amber-100/40 text-amber-700 rounded-full">
                        {{ trend.valueText }}
                      </span>
                    </div>
                    ...
                    <div class="w-full h-1.5 bg-slate-200 rounded-full overflow-hidden mt-1 shadow-inner relative">
                      ...
                      <div 
                        ...
                        :style="{ width: `${Math.min(Math.max(trend.id === 'maint_uptime' ? (trend.metricValue - 90) * 10 : trend.metricValue, 8), 100)}%` }"
                      ></div>
                    </div>
