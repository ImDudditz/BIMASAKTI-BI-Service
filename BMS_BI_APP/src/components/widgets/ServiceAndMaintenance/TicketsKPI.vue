...
const fetchTicketsKPI = async () => {
  ...
  try {
    ...
    error.value = err.response?.data?.detail || 'Failed to load Ticket KPI data. Please check backend connection.'
  } finally {
...
<template>
  ...
  <!-- Fallback error view -->
  <div v-else-if="error" class="p-4 bg-rose-50 text-rose-500 rounded-xl text-center text-sm font-medium w-full border border-rose-100">
    {{ error }}
  </div>
