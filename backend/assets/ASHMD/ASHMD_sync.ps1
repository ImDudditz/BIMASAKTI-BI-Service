...
$BackendDir = $null
While ($CurrentDir) {
    If (Test-Path (Join-Path $CurrentDir "BimasaktiReports.csproj")) {
...
If ($null -eq $BackendDir) {
    Write-Log "CRITICAL" "Could not find BimasaktiReports backend project directory."
    Exit 1
}

Write-Log "INFO" "Running standalone C# database sync via dotnet CLI..."
Try {
    $Output = dotnet run --project "$BackendDir\BimasaktiReports.csproj" -- --sync $CompanyId 2>&1
